...
- Why $R_y(\alpha)$ (rotate around the y-axis) is different?
  ...
  取决于旋转方向。简单起见，在二维直角坐标下观察，顺时针和逆时针旋转的旋转矩阵出现相同的现象。这里的$R_y(\alpha)$，旋转角是从$z$到$x$的。
  
  这里出现一个问题，旋转的正向是哪个？一般情况下，符合右手螺旋法则的旋转方向是旋转的正向（旋转角 $> 0$）：大拇指指向旋转轴的正向，四指环绕的方向就是旋转的正方向。
...
- 参考坐标系：以坐标系$D$为参考坐标系是说：我们在$D$坐标系下观察事物，所有点都表示为$D$下的坐标
- 位姿（Pose）：相机的位姿是指，在某个参考坐标系下，相机的位置$(x, y, z)$与姿态角$(roll, pitch, yaw)$。姿态角一般简记为rpy，表示了外旋的顺序。如果将rpy转换为旋转矩阵$R$（方法见「内旋与外旋」），向量$t = (x, y, z)$为相机相对参考系原点的平移量，则$T = \begin{pmatrix} R, t \\ 0^T, 1 \end{pmatrix}$称为相机的**位姿矩阵**。可以证明，$T$就是将坐标从相机坐标系换到参考坐标系（**camera-to-ref**）的变换矩阵

不难看出，MV变换中相机的位姿就是pose，只不过是表示符号不同。另外，图形学中经常规定相机看向$-z$（OpenGL。DirectX例外），而机器人（ROS）的传感器一般朝向$z$轴正向。
...
在参考坐标系$D$（目标系，当前视角）下观察相机坐标系$S$的三个轴。整个坐标变换过程涉及：平移 + 线性变换下的换系，后者要求两个系的坐标原点必须重合。因此，先平移后旋转更直观
